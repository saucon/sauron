...
func responseTimeString(startTime time.Time) (timeNano int64, timeInt int64, timeFmt string) {
	if startTime.IsZero() {
		return 0, 0, ""
	}
	timeDelta := time.Since(startTime)
	return timeDelta.Nanoseconds(), timeDelta.Milliseconds(), fmt.Sprintf("%s", timeDelta)
}
